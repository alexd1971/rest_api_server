...
main() {
  ...
  group('insert test', () {
    ...
    test('insert object', () async {
      ...
      final object = TestModel(intParam: 123, stringParam: 'some string');
  ...
  group('find tests', () {
    ...
    setUpAll(() {
      ...
      mongoCollection.insertAll([
        object1.json
          ..addAll({'_id': mongo.ObjectId.fromHexString(object1.id.json)})
          ..remove('id'),
        object2.json
          ..addAll({'_id': mongo.ObjectId.fromHexString(object2.id.json)})
          ..remove('id')
      ]);
    });
    ...
    test('find with limit', () async {
      final collection = TestMongoCollection(mongoCollection);
      final found = await collection.find(mongo.where.limit(1));
      expect(found.map((obj) => obj.json),
